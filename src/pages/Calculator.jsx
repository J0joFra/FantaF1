...
import { calculateMaxAvailablePoints, pointsNeededPerRace, isMathematicallyEliminated, MAX_POINTS_RACE, MAX_POINTS_SPRINT, getTeamColor } from "@/lib/f1Utils";
...
export default function Calculator() {
  ...
  const eliminated = driver && leader && !isLeader ? isMathematicallyEliminated(driver.points, leader.points, maxAvailable) : false;
  ...
  return (
    <div className="space-y-5 pb-10">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Seleziona un pilota per calcolare gli scenari titolo</p>
      </div>
            ...
            {drivers.map(d => (
              <SelectItem key={d.id} value={d.id}>
                <span className="flex items-center gap-2">
                  <span className="w-2 h-2 rounded-full" style={{ backgroundColor: getTeamColor(d.team) }} />
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              { label: isLeader ? "Vantaggio" : "Gap dal leader", value: isLeader ? `+${gapToSecond}` : `-${gap}` },
          ...
          {eliminated ? (
            <ScenarioCard title="Stato Campionato" icon={AlertTriangle} variant="danger">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Anche vincendo tutte le {racesLeft} gare rimaste, non può raggiungere {leader.driver_name}.
              </p>
            </ScenarioCard>
          ) : isLeader ? (
            <>
              <ScenarioCard title="Title Clinch" icon={CheckCircle2} variant="success">
                <p className="text-sm">
                  {gapToSecond > maxAvailable
                    ...
                    : `Vantaggio: ${gapToSecond} punti. Servono ${maxAvailable - gapToSecond + 1} punti di vantaggio per chiudere il titolo.`
                  }
                </p>
                {gapToSecond <= maxAvailable && (
                  <div className="mt-2 bg-secondary/50 rounded-lg p-2">
                    <p className="text-xs text-muted-foreground">
                      Punti max ancora disponibili: <strong className="text-foreground">{maxAvailable}</strong>
                    </p>
                    <div className="relative h-2 bg-secondary rounded-full mt-2 overflow-hidden">
                      <div
                        ...
                        style={{ width: `${Math.min((gapToSecond / maxAvailable) * 100, 100)}%` }}
                      />
                    </div>
                  ...
                  {drivers.slice(1, 6).map(d => {
                    ...
                    return (
                      <div key={d.id} className="flex items-center justify-between text-sm">
                        <span className={!canCatch ? "line-through text-muted-foreground" : ""}>{d.driver_name}</span>
                        <span className={`font-mono text-xs ${canCatch ? "text-foreground" : "text-muted-foreground"}`}>
                          -{dGap} pts {!canCatch && "❌"}
                        </span>
                      </div>
                    );
            ...
            <>
              <ScenarioCard title="Cosa serve per il titolo" icon={Target} variant="warning">
                <p className="text-sm">
                  {driver.driver_name} deve recuperare <strong>{gap} punti</strong> in {racesLeft} gare.
                </p>
              ...
              <ScenarioCard title="Scenario Perfetto" icon={TrendingUp}>
                <div className="space-y-2 text-sm">
                  ...
                  <ul className="space-y-1 text-xs text-muted-foreground">
                    ...
                    {sprintsLeft > 0 && <li>• {sprintsLeft} sprint vittorie = {sprintsLeft * 8} punti</li>}
                    <li>• {racesLeft} giri veloci = {racesLeft} punti</li>
                    <li className="text-foreground font-semibold pt-1">
                      Totale massimo: {maxAvailable} punti → {driver.points + maxAvailable} punti totali
                    </li>
                  </ul>
                </div>
              </ScenarioCard>
